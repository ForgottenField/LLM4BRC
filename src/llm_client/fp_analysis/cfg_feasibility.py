...
def _get_function_summary(
    ...
    """Get a formatted function summary (on-demand, cache-first).

    Lookup order:
    1. ``lib_knowledge`` static KB (for standard library functions)
    2. ``FunctionAnalyzer.summarize_function`` (cache → LLM on-demand)
    3. If none of the above → returns None (no error raised)
    """
    ...
    # ---- 1. Check static knowledge base first ----
    try:
        from llm_client.fp_analysis.lib_knowledge import lookup as kb_lookup
        kb_entry = kb_lookup(fn_name)
        if kb_entry is not None:
            return kb_entry.format_for_llm()
    except ImportError:
        pass  # lib_knowledge not available, fall through

    # ---- 2. On-demand via FunctionAnalyzer (cache + LLM) ----
    if llm_provider and source_root:
        summ_cache: SummaryCache = cache
        analyzer = FunctionAnalyzer(
            llm_provider=llm_provider,
            cache=summ_cache,
            project_name="cfg_analysis",
        )
